...
public class FunctionOutputsNode extends Node {
    ...
    public void addOutput(String name, WireType<?> type) {
        ...
        if (type == SignalType.INSTANCE) {
            output.onSignal((ctx) -> {
                if (ctx.functionScope.call == null || ctx.functionScope.parent == null) {
                    FunctionScope prev = ctx.functionScope;
                    for (FunctionCallNode call : function.callNodes) {
                        ctx.sendSignal((Output<Void>) call.getOutput(name, ctx.evaluator));
                        ctx.submit(() -> {
                            ctx.functionScope = prev;
                            prev.fakeChild(call, prev);
                        });
                    }
                    return;
                }
                ...
                ctx.sendSignal((Output<Void>) s.call.getOutput(name, ctx.evaluator));
            });
        }
    }
