...
public class PlayerCrosshairTargetNode extends Node {
    ...
    public PlayerCrosshairTargetNode() {
        ...
        entity.valueFrom((ctx) -> player.getValue(ctx).tryGet(ctx, p -> {
                    HitResult result = p.raycast(p.getAttributeValue(EntityAttributes.ENTITY_INTERACTION_RANGE), 0f, checkFluids.getValue(ctx));
                    if (!(result instanceof EntityHitResult ehit)) return new EntityValue((Entity) null);
                    return new EntityValue(ehit.getEntity());
                }, new EntityValue((Entity) null)
        ));
    }
