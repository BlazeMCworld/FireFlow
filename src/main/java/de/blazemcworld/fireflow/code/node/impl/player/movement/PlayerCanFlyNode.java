...
public class PlayerCanFlyNode extends Node {
    public PlayerCanFlyNode() {
        super("player_can_fly", "Player Can Fly", "Checks if the player can fly", Items.WOOL.white());
        Input<PlayerValue> player = new Input<>("player", "Player", PlayerType.INSTANCE);
        Output<Boolean> allowed = new Output<>("allowed", "Allowed", ConditionType.INSTANCE);

        allowed.valueFrom(ctx -> player.getValue(ctx).tryGet(ctx, p -> p.getAbilities().mayfly, false));
    }
